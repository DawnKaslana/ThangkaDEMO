...
output_path = "/home/dawn/Workspace/Django/server/media/output/"
model_path = "/home/dawn/Workspace/SDmodels/Lora"
filePath = "/home/dawn/Workspace/Django/server/media/"
...
premodel_abspath = "/home/dawn/Workspace/SDmodels/SDI2/"  # SD21/SDI2
...
pipe = StableDiffusionInpaintPipeline.from_pretrained(premodel_abspath,
                                                      ...
                                                      # variant="fp16",)
pipe.load_lora_weights(model_path+"/thangka_ACD/", weight_name="pytorch_lora_weights.safetensors")
...
def inpaint(fileName, maskName, text, steps, type, SDModel, maskImg):
    ...
    imgFile, maskFile = filePath+fileName,filePath+maskName
    image = Image.open(imgFile).resize((512,512)) # image = image.convert("RGB")
    mask_image = Image.open(maskFile).resize((512, 512))
    ...
    time.sleep(0.5)
    newimage.save(output_path+fileName[:-4]+"_output.png")
